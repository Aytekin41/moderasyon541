...
client.kisayol = new Discord.Collection();
fs.readdir("./commands/", (err, files) => {
  if (err) console.error(err);
  log(`${files.length} komut yüklenecek.`);
  files.forEach(f => {
    let props = require(`./commands/${f}`);
    log(`${props.kullanim.komut} adlı komut yüklendi. Dosya: ${f}`);
    client.commands.set(props.kullanim.komut, props);
    props.ayarlar.kisayol.forEach(alias => {
      client.kisayol.set(alias, props.kullanim.komut);
    });
  });
});
client.reload = command => {
  return new Promise((resolve, reject) => {
    try {
      ...
      client.kisayol.forEach((cmd, alias) => {
        if (cmd === command) client.kisayol.delete(alias);
      });
      client.commands.set(command, cmd);
      cmd.ayarlar.kisayol.forEach(alias => {
        client.kisayol.set(alias, cmd.kullanim.komut);
      });
      resolve();
    } catch (e) {
      reject(e);
...
client.on("guildMemberAdd", member => {
  member.roles.add("858725580659621918");
});
...
client.on("ready", async () => {
  let botVoiceChannel = client.channels.cache.get("858725684615577601");
  console.log("Bot Ses Kanalına Başarılı Bir Şekilde Bağalandı!");
  if (botVoiceChannel)
    botVoiceChannel
      .join()
    ...
    if (old.content != nev.content) {
        ...
        if (i) {
            ...
            if (küfür.some(word => nev.content.includes(word))) {
                ...
                const embed = new Discord.MessageEmbed().setColor("#ff7e00").setDescription(`${nev.author} , **Bu Sunucuda Küfür Edemezsin!**`)
                    .addField("Küfür:", nev)

                nev.delete();
                const embeds = new Discord.MessageEmbed().setColor("#ff7e00").setDescription(`${nev.author} , **Bu Sunucuda Küfür Edemezsin!**`)
        ...
        if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
            try {
                if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    const embeds = new Discord.MessageEmbed().setColor("#ff7e00").setDescription(`<@${msg.author.id}> , **Bu Sunucuda Küfür Edemezsin!**`)
                    msg.channel.send(embeds).then(msg => msg.delete({
                        timeout: 5000
                    }));
                    const embed = new Discord.MessageEmbed().setColor("#ff7e00").setDescription(`${msg.author} , **Bu Sunucuda Küfür Edemezsin!**`).addField("Mesajı:", msg)
                    client.channels.cache.get(y).send(embed)
                }
            } catch (err) {
                console.log(err);
